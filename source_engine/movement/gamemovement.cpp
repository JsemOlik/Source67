...
public:
  CDiffManager()
      : m_Client(0, 0, CDiffInfo::Less), m_Server(0, 0, CDiffInfo::Less),
        m_flLastSpew(-1.0f) {
    g_pDiffMgr = this;
  }
...
int CGameMovement::GetCheckInterval(IntervalType_t type) {
  ...
  case GROUND:
    ...
    break;
  case STUCK:
    // If we are in the process of being "stuck", then try a new position every
    // command tick until m_StuckLast gets reset back down to zero
...
#define WET_TIME			    5.f	// how many seconds till we're
completely wet/dry #define DRY_TIME			   20.f	// how many
seconds till we're completely wet/dry
...
void CGameMovement::StartGravity(void) {
  ...
  mv->m_vecVelocity[2] -=
      (ent_gravity * GetCurrentGravity() * 0.5 * gpGlobals->frametime);
...
void CGameMovement::FinishGravity(void) {
  ...
  mv->m_vecVelocity[2] -=
      (ent_gravity * GetCurrentGravity() * gpGlobals->frametime * 0.5);
...
ConVar sv_ladder_angle("sv_ladder_angle", "-0.707", FCVAR_REPLICATED,
                       "Cos of angle of incidence to ladder perpendicular for "
                       "applying ladder_dampen",
                       true, -1.0f, true, 1.0f);
#endif
//=============================================================================
// HPE_END
...
bool CGameMovement::LadderMove(void) {
  ...
  if (mv->m_nButtons & IN_JUMP) {
    ...
    if (forwardSpeed != 0 || rightSpeed != 0) {
      ...
      //=============================================================================
#if defined(CSTRIKE_DLL)
      // break lateral into direction along tmp (up the ladder) and direction
      // along perp (perpendicular to ladder)
      float tmpDist = DotProduct(tmp, lateral);
      float perpDist = DotProduct(perp, lateral);

      Vector angleVec = perp * perpDist;
      angleVec += cross;
      // angleVec is our desired movement in the ladder normal/perpendicular
      // plane
      VectorNormalize(angleVec);
      float angleDot = DotProduct(angleVec, pm.plane.normal);
      // angleDot is our angle of incidence to the laddernormal in the ladder
      // normal/perpendicular plane

      if (angleDot < sv_ladder_angle.GetFloat())
        lateral =
            (tmp * tmpDist) + (perp * sv_ladder_dampen.GetFloat() * perpDist);
#endif // CSTRIKE_DLL
       //=============================================================================
       // HPE_END
       //=============================================================================
...
void CGameMovement::AddGravity(void) {
  ...
  mv->m_vecVelocity[2] -=
      (ent_gravity * GetCurrentGravity() * gpGlobals->frametime);
  mv->m_vecVelocity[2] += player->GetBaseVelocity()[2] * gpGlobals->frametime;
  Vector temp = player->GetBaseVelocity();
  temp[2] = 0;
  player->SetBaseVelocity(temp);
...
int CGameMovement::ClipVelocity(Vector &in, Vector &normal, Vector &out,
  ...
  if (angle > 0) // If the plane that is blocking us has a positive z component,
                 // then assume it's a floor.
    blocked |= 0x01; //
...
// the original trace hit first.
//-----------------------------------------------------------------------------
void CGameMovement::TryTouchGroundInQuadrants(const Vector &start,
                                              const Vector &end,
                                              unsigned int fMask,
                                              int collisionGroup, trace_t &pm) {
...
#define NON_JUMP_VELOCITY 140.0f
  ...
  if (bMovingUpRapidly ||
    ...
    // Try and move down.
    TryTouchGround(bumpOrigin, point, GetPlayerMins(), GetPlayerMaxs(),
    ...
    if (!pm.m_pEnt || pm.plane.normal[2] < 0.7) {
      ...
      TryTouchGroundInQuadrants(bumpOrigin, point, MASK_PLAYERSOLID,
                                COLLISION_GROUP_PLAYER_MOVEMENT, pm);
...
void CGameMovement::CheckFalling(void) {
  // this function really deals with landing, not falling, so early out
  // otherwise
  if (player->GetGroundEntity() == NULL ||
...
void CGameMovement::PlayerRoughLandingEffects(float fvol) {
  if (fvol > 0.0) {
    ...
    //
    player->m_Local.m_vecPunchAngle.Set(ROLL, player->m_Local.m_flFallVelocity *
                                                  0.013);
...
#if !defined(CLIENT_DLL)
    player->RumbleEffect((fvol > 0.85f) ? (RUMBLE_FALL_LONG)
                                        : (RUMBLE_FALL_SHORT),
                         0, RUMBLE_FLAGS_NONE);
#endif
  }
...
bool CGameMovement::CanUnduck() {
  ...
  if (player->GetGroundEntity() != NULL) {
    for (i = 0; i < 3; i++) {
      newOrigin[i] += (VEC_DUCK_HULL_MIN_SCALED(player)[i] -
                       VEC_HULL_MIN_SCALED(player)[i]);
    }
  } else {
    // If in air an letting go of crouch, make sure we can offset origin to make
    //  up for uncrouching
    Vector hullSizeNormal =
        VEC_HULL_MAX_SCALED(player) - VEC_HULL_MIN_SCALED(player);
    Vector hullSizeCrouch =
        VEC_DUCK_HULL_MAX_SCALED(player) - VEC_DUCK_HULL_MIN_SCALED(player);
...
void CGameMovement::FinishDuck(void) {
  ...
  if (player->GetGroundEntity() != NULL) {
    for (int i = 0; i < 3; i++) {
      Vector org = mv->GetAbsOrigin();
      org[i] -= (VEC_DUCK_HULL_MIN_SCALED(player)[i] -
                 VEC_HULL_MIN_SCALED(player)[i]);
      mv->SetAbsOrigin(org);
    }
  } else {
...
// XBOX SERVER ONLY
      ...
      if (IsX360() && buttonsPressed & IN_DUCK) {
        // Hinting logic
        if (player->GetToggledDuckState() &&
            player->m_nNumCrouches < NUM_CROUCH_HINTS) {
...
//-----------------------------------------------------------------------------
void CGameMovement::TracePlayerBBox(const Vector &start, const Vector &end,
...
//-----------------------------------------------------------------------------
// Purpose: overridded by game classes to limit results (to standable objects
// for example)
//-----------------------------------------------------------------------------
void CGameMovement::TryTouchGround(const Vector &start, const Vector &end,

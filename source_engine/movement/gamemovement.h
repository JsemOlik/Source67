...
#define BRUSH_ONLY true
  virtual unsigned int PlayerSolidMask(
      bool brushOnly = false); ///< returns the solid mask for the given player,
                               ///< so bots can have a more-restrictive set
  CBasePlayer *player;
...
protected:
  ...
  // Useful for things that happen periodically. This lets things happen on the
  // specified interval, but spaces the events onto different frames for
  ...
  virtual float LadderDistance(void) const {
    return 2.0f;
  } ///< Returns the distance a player can be from a ladder and still attach to
    ///< it
  ...
  virtual CBaseHandle TestPlayerPosition(const Vector &pos, int collisionGroup,
                                         trace_t &pm);

...
public:
  OpenGLTexture2D(const std::string &path) : m_Path(path) {
    ...
    S67_CORE_ASSERT(file.Success, "Failed to read texture file!");

    int width, height, channels;
    stbi_set_flip_vertically_on_load(1);
    stbi_uc *data = stbi_load_from_memory(
        file.Data.data(), (int)file.Data.size(), &width, &height, &channels, 4);
    S67_CORE_ASSERT(data, "Failed to load image data!");
